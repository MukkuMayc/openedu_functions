...
function formEnrollPayload(
  ...
  a.addField("enrollment_type", enrollmentType);
...
async function formEnrollPayloadFromCourse(courseTag, sessionTag, students) {
  const university = 6;
  let res = await fetch(
  ...
  const enrollmentType = 13196;

  return formEnrollPayload(
    course,
    session,
    enrollmentType,
    students,
...
(async () => {
  for (let i = 0; i < courses.length; ++i) {
    const res = await formEnrollPayloadFromCourse(
      courses[i][0],
      courses[i][1],
